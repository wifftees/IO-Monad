...
class MyIOSpec extends AnyFunSuite with FunSuiteDiscipline with Checkers {
  implicit def monadThrowForIO[F[_]: Computation]: MonadThrow[F] = new MonadThrow[F] {
    ...
    override def handleErrorWith[A](fa: F[A])(f: Throwable => F[A]): F[A] = Computation[F].handleErrorWith(fa)(f)
  ...
  implicit def arbIntIO: Arbitrary[MyIO[Int]] = Arbitrary(
    Gen.choose(0, 100).map(MyIO.apply(_))
  )

  implicit def arbUnitIO: Arbitrary[MyIO[Unit]] = Arbitrary(
    Gen.const(MyIO.apply(()))
  )

  implicit def arbIOIntInt: Arbitrary[MyIO[Int => Int]] = Arbitrary(
    implicitly[Arbitrary[Int => Int]].arbitrary.map(MyIO.apply(_))
  )

  implicit def eqIOInt[A: Eq]: Eq[MyIO[A]] = (x: MyIO[A], y: MyIO[A]) =>
    Try(x.unsafeRunSync).toEither.eqv(Try(y.unsafeRunSync).toEither)

  implicit def eqThrowable: Eq[Throwable] = Eq.allEqual
